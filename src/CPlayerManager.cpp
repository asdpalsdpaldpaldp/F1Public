...
void CPlayerManager::FireGameEvent( IGameEvent *event )
{
	...
	auto registerPlayerEvent = [](std::unordered_map<int, playerState> &playerArray, Enum<CPlayerManager::playerMode> &playerStateEnum, IGameEvent *event)
	{
	...
	auto registerPlayerEntity = [](std::unordered_map<int, playerState> &playerArray, Enum<CPlayerManager::playerMode> &playerStateEnum, CBaseEntity *entity)
	{
		Log::Console("Player Register");
	...
	auto deregisterPlayer = [](std::unordered_map<int, playerState> &playerArray, Enum<CPlayerManager::playerMode> &playerStateEnum, IGameEvent *event)
	{
	...
	_TRY
	{
		...
		{
			deregisterPlayer(playerArray, playerStateEnum, event);
		}
		else if( !strcmp( event->GetName(), "player_connect_client" ) )
		{
			registerPlayerEvent(playerArray, playerStateEnum, event);
		}
		else if( !strcmp(event->GetName(), "teamplay_round_start"))
		{
			...
			for(int i = 0; i < gInts.EntList->GetHighestEntityIndex(); i++)
			{
				...
				{
					registerPlayerEntity(playerArray, playerStateEnum, player);
				}
			}
		}
	}
